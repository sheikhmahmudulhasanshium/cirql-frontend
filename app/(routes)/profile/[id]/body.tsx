...
import { InfoIcon, Globe2Icon } from 'lucide-react';
import { useParams } from 'next/navigation';
import Link from 'next/link';
import { ProfileActions } from '../menu-bar';

// Breakpoints (for URL truncation and screen size display)
const breakpoints = {
  phone_sm: 374,
  sm: 640,
  tablet_portrait: 767,
...
const Body = () => {
  ...
  const { data, isLoading, isError } = useUserProfile(profileID);

  // Track window width for truncation and icon label display
  ...
  // Improved URL truncation to avoid overflow between 375-480px
  const getTruncatedWebsite = (url: string): string => {
    if (width <= breakpoints.phone_sm) {
      return url.length > 15 ? url.substring(0, 15) + '...' : url;
    }
    if (width <= 480) {
      return url.length > 20 ? url.substring(0, 20) + '...' : url; // shorter than 25 chars here to avoid overflow
    }
    if (width <= breakpoints.tablet_portrait) {
      return url.length > 25 ? url.substring(0, 25) + '...' : url;
    }
    if (width <= breakpoints.desktop_sm) {
        ...
        <div className="w-full max-w-5xl flex flex-col lg:flex-row items-center lg:items-start gap-6 lg:gap-8 shadow-md rounded-lg p-6 bg-card text-card-foreground">
          ...
          <div className="flex-1 flex flex-col items-center lg:items-start text-center lg:text-left gap-4 min-w-0">
            <div className="w-full">
              ...
              <p className="text-lg text-muted-foreground break-words" title={data.headline}>
                {data.headline}
              </p>
              {data.location && (
                <p className="text-sm text-muted-foreground">From {data.location}</p>
              )}
              {data.website && (
                <Link
                  href={data.website}
                  className="flex items-center justify-center lg:justify-start gap-2 mt-2 text-primary hover:underline hover:underline-offset-4"
                  title={data.website}
                >
            ...
            {/* ProfileActions with screen size and userId */}
            <ProfileActions type="public" userId={profileID} screenWidth={width} />
          </div>
        </div>
